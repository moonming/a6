...
func NewCmdExport(f *cmd.Factory) *cobra.Command {
	...
	cmd := &cobra.Command{
		Use:   "export",
		Short: "Export stream routes as JSON or YAML",
		Args:  cobra.NoArgs,
...
func exportRun(opts *Options) error {
	...
	streamRoutes := make([]api.StreamRoute, 0, len(items))
	...
	return cmdutil.NewExporter(format, out).Write(streamRoutes)
}

func fetchAll(client *api.Client, label string) ([]api.ListItem[api.StreamRoute], error) {
	page := 1
	pageSize := 500
	items := make([]api.ListItem[api.StreamRoute], 0)
