...
func TestStreamRouteDelete_Success(t *testing.T) {
	reg := &httpmock.Registry{}
	reg.Register(http.MethodDelete, "/apisix/admin/stream_routes/1", httpmock.JSONResponse(`{"key":"/apisix/stream_routes/1","deleted":"1"}`))
	...
	assert.Contains(t, stdout.String(), "Stream route 1 deleted.")
	reg.Verify(t)
}

func TestStreamRouteDelete_NotFound(t *testing.T) {
	reg := &httpmock.Registry{}
	reg.Register(http.MethodDelete, "/apisix/admin/stream_routes/999", httpmock.StringResponse(404, `{"error_msg":"not found"}`))
...
func TestStreamrouteDelete_NoArgsNonTTY(t *testing.T) {
	ios, _, _, _ := iostreams.Test()
...
func TestStreamrouteDelete_AllAndLabelMutuallyExclusive(t *testing.T) {
	ios, _, _, _ := iostreams.Test()
...
func TestStreamrouteDelete_AllWithID(t *testing.T) {
	ios, _, _, _ := iostreams.Test()
	f := &cmd.Factory{IOStreams: ios}
...
func TestStreamrouteDelete_LabelWithID(t *testing.T) {
	ios, _, _, _ := iostreams.Test()
	f := &cmd.Factory{IOStreams: ios}
