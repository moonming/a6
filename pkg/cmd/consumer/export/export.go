...
func NewCmdExport(f *cmd.Factory) *cobra.Command {
	...
	cmd := &cobra.Command{
		Use:   "export",
		Short: "Export consumers as JSON or YAML",
		Args:  cobra.NoArgs,
...
func exportRun(opts *Options) error {
	...
	consumers := make([]api.Consumer, 0, len(items))
	...
	return cmdutil.NewExporter(format, out).Write(consumers)
}

func fetchAll(client *api.Client, label string) ([]api.ListItem[api.Consumer], error) {
	page := 1
	pageSize := 500
