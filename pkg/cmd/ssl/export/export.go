...
func NewCmdExport(f *cmd.Factory) *cobra.Command {
	...
	cmd := &cobra.Command{
		Use:   "export",
		Short: "Export SSL certificates as JSON or YAML",
		Args:  cobra.NoArgs,
...
func exportRun(opts *Options) error {
	...
	return cmdutil.NewExporter(format, out).Write(ssls)
}

func fetchAll(client *api.Client, label string) ([]api.ListItem[api.SSL], error) {
	page := 1
	pageSize := 500
	items := make([]api.ListItem[api.SSL], 0)
