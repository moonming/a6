...
func NewCmdCreate(f *cmd.Factory) *cobra.Command {
	...
	cmd := &cobra.Command{
		Use:   "create",
		Short: "Create an SSL certificate from a file",
...
func createRun(opts *Options) error {
	...
	if id, ok := payload["id"]; ok {
		...
		body, err = client.Put(fmt.Sprintf("/apisix/admin/ssls/%s", idStr), payload)
	} else {
		body, err = client.Post("/apisix/admin/ssls", payload)
	}
	if err != nil {
		return fmt.Errorf("%s", cmdutil.FormatAPIError(err))
	}

	var resp api.SingleResponse[api.SSL]
