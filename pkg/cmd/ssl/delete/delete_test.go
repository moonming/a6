...
func TestSSLDelete_Force(t *testing.T) {
	reg := &httpmock.Registry{}
	reg.Register(http.MethodDelete, "/apisix/admin/ssls/test-ssl-1", httpmock.JSONResponse(`{"key":"/apisix/ssls/test-ssl-1","deleted":"1"}`))
	...
	c := NewCmdDelete(f)
	c.SetArgs([]string{"test-ssl-1", "--force"})
	err := c.Execute()

	require.NoError(t, err)
	assert.Contains(t, stdout.String(), "test-ssl-1")
	assert.Contains(t, stdout.String(), "deleted")
	reg.Verify(t)
}

func TestSSLDelete_NotFound(t *testing.T) {
	reg := &httpmock.Registry{}
	reg.Register(http.MethodDelete, "/apisix/admin/ssls/nonexistent", httpmock.StringResponse(404, `{"error_msg":"not found"}`))
