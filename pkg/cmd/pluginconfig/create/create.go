...
func NewCmdCreate(f *cmd.Factory) *cobra.Command {
	...
	cmd := &cobra.Command{
		Use:   "create",
		Short: "Create a plugin config from a file",
...
func createRun(opts *Options) error {
	...
	idRaw, ok := payload["id"]
	if !ok {
		return fmt.Errorf("plugin config configuration must include an \"id\" field")
	}
	idStr := fmt.Sprintf("%v", idRaw)

	client := api.NewClient(httpClient, cfg.BaseURL())

	body, err := client.Put(fmt.Sprintf("/apisix/admin/plugin_configs/%s", idStr), payload)
	if err != nil {
