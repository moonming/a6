...
func NewCmdExport(f *cmd.Factory) *cobra.Command {
	...
	cmd := &cobra.Command{
		Use:   "export",
		Short: "Export plugin configs as JSON or YAML",
		Args:  cobra.NoArgs,
...
func exportRun(opts *Options) error {
	...
	pluginConfigs := make([]api.PluginConfig, 0, len(items))
	...
	return cmdutil.NewExporter(format, out).Write(pluginConfigs)
}

func fetchAll(client *api.Client, label string) ([]api.ListItem[api.PluginConfig], error) {
	page := 1
	pageSize := 500
	items := make([]api.ListItem[api.PluginConfig], 0)
