...
func NewCmdDelete(f *cmd.Factory) *cobra.Command {
	...
	cmd.Flags().BoolVar(&opts.All, "all", false, "Delete all plugin configs")
	cmd.Flags().StringVar(&opts.Label, "label", "", "Delete plugin configs matching label (key=value)")
...
func deleteRun(opts *Options) error {
	if opts.All || opts.Label != "" {
		return bulkDeletePluginConfigs(opts)
	}
	...
	_, err = client.Delete(fmt.Sprintf("/apisix/admin/plugin_configs/%s", opts.ID), nil)
	if err != nil {
...
func bulkDeletePluginConfigs(opts *Options) error {
	cfg, err := opts.Config()
	if err != nil {
		return err
	}
	...
	client := api.NewClient(httpClient, cfg.BaseURL())
	ids, err := listAllPluginConfigIDs(client, opts.Label)
	if err != nil {
		return fmt.Errorf("%s", cmdutil.FormatAPIError(err))
	}

	if len(ids) == 0 {
		fmt.Fprintln(opts.IO.ErrOut, "No plugin configs found.")
		return nil
	}

	if !opts.Force && opts.IO.IsStdinTTY() {
		fmt.Fprintf(opts.IO.ErrOut, "Delete %d plugin configs? (y/N): ", len(ids))
		reader := bufio.NewReader(opts.IO.In)
		answer, err := reader.ReadString('\n')
		if err != nil {
			return fmt.Errorf("failed to read confirmation: %w", err)
	...
	for _, id := range ids {
		_, err := client.Delete(fmt.Sprintf("/apisix/admin/plugin_configs/%s", id), nil)
		if err != nil {
			return fmt.Errorf("%s", cmdutil.FormatAPIError(err))
		}
		fmt.Fprintf(opts.IO.Out, "✓ Plugin config %s deleted.\n", id)
	...
	fmt.Fprintf(opts.IO.Out, "✓ %d plugin configs deleted.\n", len(ids))
	return nil
}

func listAllPluginConfigIDs(client *api.Client, label string) ([]string, error) {
	page := 1
	pageSize := 500
	ids := make([]string, 0)
