...
func TestPluginConfigDelete_Success(t *testing.T) {
	reg := &httpmock.Registry{}
	reg.Register(http.MethodDelete, "/apisix/admin/plugin_configs/1", httpmock.JSONResponse(`{"key":"/apisix/plugin_configs/1","deleted":"1"}`))
	...
	assert.Contains(t, stdout.String(), "Plugin config 1 deleted.")
	reg.Verify(t)
}

func TestPluginConfigDelete_NotFound(t *testing.T) {
	reg := &httpmock.Registry{}
	reg.Register(http.MethodDelete, "/apisix/admin/plugin_configs/999", httpmock.StringResponse(404, `{"error_msg":"not found"}`))
