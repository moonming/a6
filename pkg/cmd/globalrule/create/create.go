...
func NewCmdCreate(f *cmd.Factory) *cobra.Command {
	...
	cmd := &cobra.Command{
		Use:   "create",
		Short: "Create a global rule from a file",
...
func createRun(opts *Options) error {
	...
	idRaw, ok := payload["id"]
	if !ok {
		return fmt.Errorf("global rule configuration must include an \"id\" field")
	}
	idStr := fmt.Sprintf("%v", idRaw)

	body, err := client.Put(fmt.Sprintf("/apisix/admin/global_rules/%s", idStr), payload)
	if err != nil {
		return fmt.Errorf("%s", cmdutil.FormatAPIError(err))
	}

	var resp api.SingleResponse[api.GlobalRule]
