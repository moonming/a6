...
func TestGlobalRuleExport_WithLabelFilter(t *testing.T) {
	...
	transport := roundTripperFunc(func(req *http.Request) (*http.Response, error) {
		if req.Method == http.MethodGet && req.URL.Path == "/apisix/admin/global_rules" {
			if req.URL.Query().Get("label") == "scope:global" {
				calledWithNormalizedLabel = true
			}
			if strings.Contains(req.URL.RawQuery, "label=scope:global") {
				rawQueryContainsUnescapedColon = true
			}
			return &http.Response{
				StatusCode: 200,
				Header:     http.Header{"Content-Type": []string{"application/json"}},
				Body:       io.NopCloser(strings.NewReader(`{"total":1,"list":[{"key":"/apisix/global_rules/gr1","value":{"id":"gr1","plugins":{"prometheus":{}}}}]}`)),
			}, nil
		}
		return &http.Response{StatusCode: 404, Body: io.NopCloser(strings.NewReader(`{"error_msg":"not found"}`))}, nil
	})
