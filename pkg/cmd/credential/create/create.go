...
func NewCmdCreate(f *cmd.Factory) *cobra.Command {
	...
	cmd := &cobra.Command{
		Use:   "create",
		Short: "Create a consumer credential from a file",
...
func createRun(opts *Options) error {
	...
	idRaw, ok := payload["id"]
	if !ok {
		return fmt.Errorf("credential configuration must include an \"id\" field")
	}
	id := fmt.Sprintf("%v", idRaw)

	client := api.NewClient(httpClient, cfg.BaseURL())
	body, err := client.Put(fmt.Sprintf("/apisix/admin/consumers/%s/credentials/%s", opts.Consumer, id), payload)
	if err != nil {
		return fmt.Errorf("%s", cmdutil.FormatAPIError(err))
	}

	var resp api.SingleResponse[api.Credential]
