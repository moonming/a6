...
func TestServiceDelete_WithForce(t *testing.T) {
	reg := &httpmock.Registry{}
	reg.Register(http.MethodDelete, "/apisix/admin/services/1", httpmock.JSONResponse(`{"key":"/apisix/services/1","deleted":"1"}`))
	...
	assert.Contains(t, stdout.String(), "Service 1 deleted.")
	reg.Verify(t)
}

func TestServiceDelete_NotFound(t *testing.T) {
	reg := &httpmock.Registry{}
	reg.Register(http.MethodDelete, "/apisix/admin/services/999", httpmock.StringResponse(404, `{"error_msg":"not found"}`))
...
func TestServiceDelete_MissingArg(t *testing.T) {
	ios, _, _, _ := iostreams.Test()
...
func TestServiceDelete_AllAndLabelMutuallyExclusive(t *testing.T) {
	ios, _, _, _ := iostreams.Test()
...
func TestServiceDelete_AllWithID(t *testing.T) {
	ios, _, _, _ := iostreams.Test()
	f := &cmd.Factory{IOStreams: ios}
...
func TestServiceDelete_LabelWithID(t *testing.T) {
	ios, _, _, _ := iostreams.Test()
	f := &cmd.Factory{IOStreams: ios}
