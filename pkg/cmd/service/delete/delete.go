...
func NewCmdDelete(f *cmd.Factory) *cobra.Command {
	...
	cmd.Flags().BoolVar(&opts.All, "all", false, "Delete all services")
	cmd.Flags().StringVar(&opts.Label, "label", "", "Delete services matching label (key=value)")
...
func deleteRun(opts *Options) error {
	if opts.All || opts.Label != "" {
		return bulkDeleteServices(opts)
	}
	...
	if !opts.Force && opts.IO.IsStdinTTY() {
		fmt.Fprintf(opts.IO.ErrOut, "Delete service %s? (y/N): ", opts.ID)
	...
	_, err = client.Delete(fmt.Sprintf("/apisix/admin/services/%s", opts.ID), nil)
	if err != nil {
		return fmt.Errorf("%s", cmdutil.FormatAPIError(err))
	}

	fmt.Fprintf(opts.IO.Out, "✓ Service %s deleted.\n", opts.ID)
	return nil
}

func bulkDeleteServices(opts *Options) error {
	cfg, err := opts.Config()
	if err != nil {
		return err
	}
	...
	ids, err := listAllServiceIDs(client, opts.Label)
	if err != nil {
		return fmt.Errorf("%s", cmdutil.FormatAPIError(err))
	}
	...
	if !opts.Force && opts.IO.IsStdinTTY() {
		fmt.Fprintf(opts.IO.ErrOut, "Delete %d services? (y/N): ", len(ids))
		reader := bufio.NewReader(opts.IO.In)
		answer, err := reader.ReadString('\n')
		if err != nil {
			return fmt.Errorf("failed to read confirmation: %w", err)
	...
	for _, id := range ids {
		_, err := client.Delete(fmt.Sprintf("/apisix/admin/services/%s", id), nil)
		if err != nil {
			return fmt.Errorf("%s", cmdutil.FormatAPIError(err))
		}
		fmt.Fprintf(opts.IO.Out, "✓ Service %s deleted.\n", id)
	...
	fmt.Fprintf(opts.IO.Out, "✓ %d services deleted.\n", len(ids))
	return nil
}

func listAllServiceIDs(client *api.Client, label string) ([]string, error) {
	page := 1
	pageSize := 500
	ids := make([]string, 0)
...
func selectService(opts *Options) (string, error) {
	cfg, err := opts.Config()
	if err != nil {
		return "", err
	}
	...
	client := api.NewClient(httpClient, cfg.BaseURL())
	body, err := client.Get("/apisix/admin/services", nil)
	if err != nil {
		return "", fmt.Errorf("failed to fetch services: %s", cmdutil.FormatAPIError(err))
	}
	...
	return selector.SelectOne("Select a service", items)
}
