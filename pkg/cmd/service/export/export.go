...
func exportRun(opts *Options) error {
	...
	return cmdutil.NewExporter(format, out).Write(services)
}

func fetchAll(client *api.Client, label string) ([]api.ListItem[api.Service], error) {
	page := 1
	pageSize := 500
	items := make([]api.ListItem[api.Service], 0)
