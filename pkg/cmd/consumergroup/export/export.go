...
func NewCmdExport(f *cmd.Factory) *cobra.Command {
	...
	cmd := &cobra.Command{
		Use:   "export",
		Short: "Export consumer groups as JSON or YAML",
		Args:  cobra.NoArgs,
...
func exportRun(opts *Options) error {
	...
	consumerGroups := make([]api.ConsumerGroup, 0, len(items))
	...
	return cmdutil.NewExporter(format, out).Write(consumerGroups)
}

func fetchAll(client *api.Client, label string) ([]api.ListItem[api.ConsumerGroup], error) {
	page := 1
	pageSize := 500
	items := make([]api.ListItem[api.ConsumerGroup], 0)
