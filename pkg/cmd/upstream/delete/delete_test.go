...
func TestUpstreamDelete_Force(t *testing.T) {
	reg := &httpmock.Registry{}
	reg.Register(http.MethodDelete, "/apisix/admin/upstreams/test-ups-1", httpmock.JSONResponse(`{"key":"/apisix/upstreams/test-ups-1","deleted":"1"}`))
	...
	c := NewCmdDelete(f)
	c.SetArgs([]string{"test-ups-1", "--force"})
	err := c.Execute()

	require.NoError(t, err)
	assert.Contains(t, stdout.String(), "test-ups-1")
	assert.Contains(t, stdout.String(), "deleted")
	reg.Verify(t)
}

func TestUpstreamDelete_NotFound(t *testing.T) {
	reg := &httpmock.Registry{}
	reg.Register(http.MethodDelete, "/apisix/admin/upstreams/nonexistent", httpmock.StringResponse(404, `{"error_msg":"not found"}`))
