...
func NewCmdDelete(f *cmd.Factory) *cobra.Command {
	...
	cmd.Flags().BoolVar(&opts.All, "all", false, "Delete all upstreams")
	cmd.Flags().StringVar(&opts.Label, "label", "", "Delete upstreams matching label (key=value)")
...
func deleteRun(opts *Options) error {
	if opts.All || opts.Label != "" {
		return bulkDeleteUpstreams(opts)
	}
...
func bulkDeleteUpstreams(opts *Options) error {
	cfg, err := opts.Config()
	if err != nil {
		return err
	}
	...
	ids, err := listAllUpstreamIDs(client, opts.Label)
	if err != nil {
		return fmt.Errorf("%s", cmdutil.FormatAPIError(err))
	}
	...
	if !opts.Force && opts.IO.IsStdinTTY() {
		fmt.Fprintf(opts.IO.ErrOut, "Delete %d upstreams? (y/N): ", len(ids))
		reader := bufio.NewReader(opts.IO.In)
		answer, err := reader.ReadString('\n')
		if err != nil {
			return fmt.Errorf("failed to read confirmation: %w", err)
	...
	for _, id := range ids {
		_, err := client.Delete(fmt.Sprintf("/apisix/admin/upstreams/%s", id), nil)
		if err != nil {
			return fmt.Errorf("%s", cmdutil.FormatAPIError(err))
		}
		fmt.Fprintf(opts.IO.Out, "✓ Upstream %s deleted.\n", id)
	...
	fmt.Fprintf(opts.IO.Out, "✓ %d upstreams deleted.\n", len(ids))
	return nil
}

func listAllUpstreamIDs(client *api.Client, label string) ([]string, error) {
	page := 1
	pageSize := 500
	ids := make([]string, 0)
