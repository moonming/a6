...
func TestProtoExport_BasicYAML(t *testing.T) {
	transport := roundTripperFunc(func(req *http.Request) (*http.Response, error) {
		if req.Method == http.MethodGet && req.URL.Path == "/apisix/admin/protos" {
			return &http.Response{
				StatusCode: 200,
				Header:     http.Header{"Content-Type": []string{"application/json"}},
				Body:       io.NopCloser(strings.NewReader(`{"total":1,"list":[{"key":"/apisix/protos/p1","value":{"id":"p1","content":"syntax = \"proto3\";","name":"grpc-proto","desc":"gRPC service definition","create_time":1,"update_time":2}}]}`)),
			}, nil
		}
...
func TestProtoExport_WithLabelFilter(t *testing.T) {
	...
	transport := roundTripperFunc(func(req *http.Request) (*http.Response, error) {
		if req.Method == http.MethodGet && req.URL.Path == "/apisix/admin/protos" {
			if req.URL.Query().Get("label") == "type" {
				calledWithLabelKey = true
			}
			return &http.Response{
				StatusCode: 200,
				Header:     http.Header{"Content-Type": []string{"application/json"}},
				Body:       io.NopCloser(strings.NewReader(`{"total":1,"list":[{"key":"/apisix/protos/p1","value":{"id":"p1","content":"syntax = \"proto3\";","name":"grpc-proto","labels":{"type":"grpc"}}}]}`)),
			}, nil
		}
		return &http.Response{StatusCode: 404, Body: io.NopCloser(strings.NewReader(`{"error_msg":"not found"}`))}, nil
	})
	...
	c := NewCmdExport(f)
	c.SetArgs([]string{"--label", "type=grpc", "--output", "json"})
	err := c.Execute()
...
func TestProtoExport_EmptyResult(t *testing.T) {
	transport := roundTripperFunc(func(req *http.Request) (*http.Response, error) {
		if req.Method == http.MethodGet && req.URL.Path == "/apisix/admin/protos" {
			return &http.Response{
				StatusCode: 200,
	...
	assert.Contains(t, stderr.String(), "No proto definitions found.")
}

func TestProtoExport_APIError(t *testing.T) {
	transport := roundTripperFunc(func(req *http.Request) (*http.Response, error) {
		return &http.Response{
			StatusCode: 500,
			Header:     http.Header{"Content-Type": []string{"application/json"}},
...
func TestProtoExport_CorrectAPIPath(t *testing.T) {
	...
	assert.Equal(t, "/apisix/admin/protos", requestedPath)
}
