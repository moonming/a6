...
func NewCmdExport(f *cmd.Factory) *cobra.Command {
	...
	cmd := &cobra.Command{
		Use:   "export",
		Short: "Export proto definitions as JSON or YAML",
		Args:  cobra.NoArgs,
...
func exportRun(opts *Options) error {
	...
	protos := make([]api.Proto, 0, len(items))
	...
	return cmdutil.NewExporter(format, out).Write(protos)
}

func fetchAll(client *api.Client, label string) ([]api.ListItem[api.Proto], error) {
	page := 1
	pageSize := 500
	items := make([]api.ListItem[api.Proto], 0)
