...
func TestProtoDelete_Success(t *testing.T) {
	reg := &httpmock.Registry{}
	reg.Register(http.MethodDelete, "/apisix/admin/protos/1", httpmock.JSONResponse(`{"key":"/apisix/protos/1","deleted":"1"}`))
	...
	assert.Contains(t, stdout.String(), "Proto 1 deleted.")
	reg.Verify(t)
}

func TestProtoDelete_NotFound(t *testing.T) {
	reg := &httpmock.Registry{}
	reg.Register(http.MethodDelete, "/apisix/admin/protos/999", httpmock.StringResponse(404, `{"error_msg":"not found"}`))
...
func TestProtoDelete_NoArgsNonTTY(t *testing.T) {
	ios, _, _, _ := iostreams.Test()
...
func TestProtoDelete_AllAndLabelMutuallyExclusive(t *testing.T) {
	ios, _, _, _ := iostreams.Test()
...
func TestProtoDelete_AllWithID(t *testing.T) {
	ios, _, _, _ := iostreams.Test()
	f := &cmd.Factory{IOStreams: ios}
...
func TestProtoDelete_LabelWithID(t *testing.T) {
	ios, _, _, _ := iostreams.Test()
	f := &cmd.Factory{IOStreams: ios}
